    ...
    if (Request.method == "POST"):
        ...
        if (p == cp):
            ...
            if (user):
                ...
                message = 'Hello'+" "+b.name+'\nThank to create a buyer account with us\nNow you can Buy your Latest Products\nTeam Eshop :'
...
def searchPage(Request):
    ...
    data = Product.objects.filter(Q(name__icontains=search)|Q(color__icontains=search)|Q(size__icontains=search)|Q(stock__icontains=search)|Q(description__icontains=search))
    maincategory = Maincategory.objects.all()
    subcategory = Subcategory.objects.all()
    brand = Brand.objects.all()
    ...
    if(Request.method=="POST"):
        ...
        try:
            ...
            if(user.is_superuser):
                ...
                buyer.otp=otp # otp svae into buyer of otp
                buyer.save()
                subject = 'OTP for password Reset : Team Eshop '
                message =  'OTP for password Reset: ' +str(otp) +"\nTeam Eshop"
                email_from = settings.EMAIL_HOST_USER
